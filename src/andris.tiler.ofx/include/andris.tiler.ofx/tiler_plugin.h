...
public:
  tiler_plugin(OfxImageEffectHandle handle) noexcept;

  void render(OFX::RenderArguments const &args) noexcept override;

  bool isIdentity(OFX::IsIdentityArguments const &args,
                  OFX::Clip *&identityClip,
                  double &identityTime) noexcept override;

  void changedParam(OFX::InstanceChangedArgs const &args,
...
private:
  ...
  OFX::DoubleParam *horizontal_scale{
      fetchDoubleParam(tiler::param_horizontal_scale)};
  OFX::DoubleParam *vertical_scale{
      fetchDoubleParam(tiler::param_vertical_scale)};
