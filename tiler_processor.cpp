...
tiler_processor::tiler_processor(
	OFX::ImageEffect& effect) noexcept : OFX::ImageProcessor(effect)
{
}

void tiler_processor::set_source(
	...
	OfxRectI window) 
{
	...
	auto source_mat{ cv::Mat(
		source_bounds.y2 - source_bounds.y1,
		source_bounds.x2 - source_bounds.x1,
		source_type,
		source_data,
		source_row_bytes) };
	...
	auto output_mat{ cv::Mat(
		output_bounds.y2 - output_bounds.y1,
		output_bounds.x2 - output_bounds.x1,
		output_type,
		output_data,
		output_row_bytes) };
...
void tiler_processor::set_center(
	OfxPointD const& value) noexcept
{
	center = value;
}

void tiler_processor::set_horizontal_scale(
	double value) noexcept
{
	horizontal_scale = value;
}

void tiler_processor::set_vertical_scale(
	double value) noexcept
{
	...
	OfxRectI const& window, cv::Mat const& input, cv::Mat& output) 
{
	...
	auto x_iterations_count{ static_cast<int>(std::ceil(x_diff / static_cast<double>(input.cols))) };
	auto x{ x_diff - (x_iterations_count * input.cols) };

	auto y_diff{ y_offset - util::nearest_even(input.rows / 2.0) - window.y1 };
	auto y_iterations_count{ static_cast<int>(std::ceil(y_diff / static_cast<double>(input.rows))) };
